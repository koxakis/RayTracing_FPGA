...
public:
    ...
    // call the getGeometryData() method of the intersected mesh to compute the normal and the texture coordinates at the intersection point.
...
// The function readGeometryFile() returns a pointer to a new dynamically allocated instance of the TriangleMesh class.
...
// function which is where we loop over all the objects contained in the objects list.
bool trace(
    const Vec3f &orig, const Vec3f &dir,
...
// perform shading in the castRay() function 
Vec3f castRay(
    const Vec3f &orig, const Vec3f &dir,
    const std::vector<std::unique_ptr<Object>> &objects,
    const Options &options)
{
    ...
    if (trace(orig, dir, objects, tnear, index, uv, &hitObject)) {
        ...
        // create the checkerboard pattern and compute the facing-ratio which mixed together form the final pixel color 
